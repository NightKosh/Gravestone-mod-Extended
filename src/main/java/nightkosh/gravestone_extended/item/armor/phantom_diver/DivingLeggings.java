 ...
 */
public class DivingLeggings extends Item {

    private static final ResourceKey<Item> RK = ResourceKey.create(
            Registries.ITEM, fromNamespaceAndPath(ModInfo.ID, "diving_leggings"));
    ...
    public void inventoryTick(@Nonnull ItemStack stack, @Nonnull ServerLevel level,
        ...
        if (entity instanceof Player player &&
                player.getItemBySlot(EquipmentSlot.LEGS).equals(stack)) {
            if (!level.isClientSide() && player.isUnderWater() && player.tickCount % 20 == 0) {
                player.addEffect(new MobEffectInstance(MobEffects.DOLPHINS_GRACE, TimeHelper.SECONDS_5,
                        0, true, false, true));
            }
        }
    }
    ...
    public void appendHoverText(
        ...
        consumer.accept(Component.translatable("item.gravestone_extended.diving_leggings.tooltip"));
